...
- `.github/workflows/release.yml` triggers on `v*.*.*` tags, builds, packs, creates a GitHub release, and uploads packages as artifacts.
- `.github/workflows/publish-nuget.yml` triggers on a published release and pushes artifacts to NuGet.org using the `NUGET_API_KEY` secret.
